                    ...
                    <div className="space-y-2">
                        {config.footer.usefulLinks.map((item, i) => (
                            <div key={i} className="flex gap-2 group">
                                <input value={item.label} onChange={e => {
                                    const newList = [...config.footer.usefulLinks]; newList[i].label = e.target.value; setConfig({ ...config, footer: { ...config.footer, usefulLinks: newList } })
                                }} className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-white" placeholder="Lien" />
                                <button onClick={() => removeFooterLink('usefulLinks', i)} className="text-slate-500 p-1 opacity-0 group-hover:opacity-100"><Trash2 className="w-3.5 h-3.5" /></button>
                            </div>
                        ))}
                    </div>
                </section>
                ...
                <section className="bg-slate-800/50 p-8 rounded-2xl border border-slate-700/50 shadow-sm">
                    ...
                    <div className="space-y-2">
                        {config.footer.masterLinks.map((item, i) => (
                            <div key={i} className="flex gap-2 group">
                                <input value={item.label} onChange={e => {
                                    const newList = [...config.footer.masterLinks]; newList[i].label = e.target.value; setConfig({ ...config, footer: { ...config.footer, masterLinks: newList } })
                                }} className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-1.5 text-xs text-white" placeholder="Lien" />
                                <button onClick={() => removeFooterLink('masterLinks', i)} className="text-slate-500 p-1 opacity-0 group-hover:opacity-100"><Trash2 className="w-3.5 h-3.5" /></button>
                            </div>
                        ))}
                    </div>
                </section>
