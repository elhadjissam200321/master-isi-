        ...
        <div className="p-8 max-w-6xl mx-auto pb-24 text-slate-200">
            ...
            <div className="flex gap-6">
                ...
                <div className="flex-1 space-y-12 min-w-0">
                    {activeTab === "hero" && (
                        <section className="bg-slate-800/50 p-8 rounded-2xl border border-slate-700/50 shadow-sm space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                            ...
                            <div className="pt-6 border-t border-slate-700">
                                <h3 className="text-sm font-bold text-slate-300 mb-4">Informations rapides</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {data.quickInfo.map((info, i) => (
                                        <div key={i} className="bg-slate-900 p-4 rounded-xl border border-slate-700 flex gap-4 items-center">
                                            <div className="bg-slate-800 p-2.5 rounded-lg border border-slate-700">
                                                <Cpu className="w-4 h-4 text-indigo-400" />
                                            </div>
                                            <div className="flex-1">
                                                <label className="text-[10px] text-slate-500 uppercase block">{info.label}</label>
                                                <input
                                                    value={info.value}
                                                    onChange={e => {
                                                        const next = [...data.quickInfo]
                                                        next[i].value = e.target.value
                                                        setData({ ...data, quickInfo: next })
                                                    }}
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                            <input
                                                ...
                                                onChange={e => {
                                                    ...
                                                    next[semKey].name = e.target.value
                                                    setData({ ...data, semesterModules: next })
                                                }}
                                                className="bg-slate-900 border border-slate-700 rounded-xl px-4 py-2 text-white outline-none focus:border-indigo-500 transition"
                                                placeholder="Titre du semestre"
                                            />
                                            <input
                                                value={sem.description}
                                                onChange={e => {
                                                    const next = { ...data.semesterModules }
                                                    next[semKey].description = e.target.value
                                                    setData({ ...data, semesterModules: next })
                                                }}
                                                className="bg-slate-900 border border-slate-700 rounded-xl px-4 py-2 text-white outline-none focus:border-indigo-500 transition"
                                        ...
                                        {!sem.options ? (
                                            <div className="space-y-4">
                                                <div className="flex justify-between items-center mb-2">
                                                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Liste des modules</h3>
                                                    <button
                                                        onClick={() => {
                                                            const next = { ...data.semesterModules }
                                                            next[semKey].modules = [...next[semKey].modules, { name: "Nouveau module", prof: "Période", icon: "Code" }]
                                                            setData({ ...data, semesterModules: next })
                                                        }}
                                                        className="bg-indigo-500/10 text-indigo-400 px-3 py-1 rounded text-[10px] font-bold hover:bg-indigo-500/20 transition"
                                                    >
                                                        + AJOUTER MODULE
                                                    </button>
                                                </div>
                                                    ...
                                                    <div key={opt.key} className="bg-slate-900/30 p-6 rounded-2xl border border-indigo-500/20">
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                                            <input
                                                                ...
                                                                onChange={e => {
                                                                    const next = { ...data.semesterModules }
                                                                    next[semKey].options![optIdx].name = e.target.value
                                                                    setData({ ...data, semesterModules: next })
                                                                }}
                                                                className="bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white outline-none focus:border-indigo-500 text-sm"
                                                            />
                                                            <input
                                                                value={opt.color}
                                                                onChange={e => {
                                                                    const next = { ...data.semesterModules }
                                                                    next[semKey].options![optIdx].color = e.target.value
                                                                    setData({ ...data, semesterModules: next })
                                                                }}
                                                                className="bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white outline-none focus:border-indigo-500 text-sm"
                                                                placeholder="Classe CSS couleur (ex: bg-blue-600)"
                                                            />
                                                        </div>
                                                        <div className="space-y-2">
                                                            <div className="flex justify-between items-center mb-2">
                                                                <span className="text-[10px] font-bold text-slate-500 uppercase">Modules spécialisés</span>
                                                                <button
                                                                    onClick={() => {
                                                                        const next = { ...data.semesterModules }
                                                                        next[semKey].options![optIdx].modules = [...next[semKey].options![optIdx].modules, { name: "Nouveau...", prof: "Pr.", icon: "Code" }]
                                                                        setData({ ...data, semesterModules: next })
                                                                    }}
                                                ...
                                                <div className="bg-indigo-500/5 p-6 rounded-2xl border border-indigo-500/20">
                                                    <h4 className="text-sm font-bold text-indigo-400 mb-4 flex justify-between items-center">
                                                        ...
                                                        <button
                                                            onClick={() => {
                                                                const next = { ...data.semesterModules }
                                                                next[semKey].commonModules = [...(next[semKey].commonModules || []), { name: "Commun...", prof: "Pr.", icon: "Network" }]
                                                                setData({ ...data, semesterModules: next })
                                                            }}
                                                            className="text-[10px] text-indigo-400 font-bold"
                                                        >
                    ...
                    {activeTab === "s4" && (
                        <section className="bg-slate-800/50 p-8 rounded-2xl border border-slate-700/50 shadow-sm space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                            <h2 className="text-xl font-bold text-white border-b border-slate-700 pb-3 mb-6">Contenu du Semestre 4</h2>
                            <div className="grid grid-cols-1 gap-6">
                                ...
                                <div className="space-y-3 pt-4">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <button
                                            onClick={() => setData({ ...data, s4Section: { ...data.s4Section, items: [...data.s4Section.items, "Nouvel élément..."] } })}
                                    ...
                                    {data.s4Section.items.map((item, i) => (
                                        <div key={i} className="flex gap-2">
                                            ...
                                            <button onClick={() => setData({ ...data, s4Section: { ...data.s4Section, items: data.s4Section.items.filter((_, idx) => idx !== i) } })} className="p-2 text-slate-600 hover:text-red-400">
                                                <Trash2 className="w-4 h-4" />
                                            </button>
                                        </div>
                                    ))}
